...
class Stage2TransportTrainer:
    ...
    def _split_dataset(self, dataset: Dataset) -> tuple[Dataset, Dataset, dict]:
        ...
        elif strategy in {"leave_some_holes_out", "leave_fixed_holes_out"}:
            hole_col = str(vcfg.get("hole_column", "hole_id"))
            fixed_holes = vcfg.get("holdout_holes") if strategy == "leave_fixed_holes_out" else None
            if fixed_holes:
                holdout_holes = {int(h) for h in fixed_holes}
                n_holdout = len(holdout_holes)
            else:
